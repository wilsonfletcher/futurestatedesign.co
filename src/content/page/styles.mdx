---
name: Styles
draft: true
---

import { Icon } from 'astro-icon'
import { Banner, Prose, Article, Figure } from '@/components/index.astro';

export const authors = [
  { name: 'Jane', email: 'hi@jane.com' },
  { name: 'John', twitter: '@john2002' },
];

export const published = new Date('2022-02-01');

export const components = { code: (props) => <span class="block text-red-light" {...props }/>}

<Banner>
  # Styles
</Banner>

<Article>
  ## Icons

  <div class="flex flex-wrap gap-4">
  <Icon name="arrow/45/24" class="bg-slate-200" />
  <Icon name="arrow/45/32" class="bg-slate-200"/>
  <Icon name="arrow/45/48" class="bg-slate-200"/>
  <Icon name="arrow/45/52" class="bg-slate-200"/>
  <Icon name="arrow/90/24" class="bg-slate-200"/>
  <Icon name="arrow/90/32" class="bg-slate-200"/>
  <Icon name="arrow/90/48" class="bg-slate-200"/>
  <Icon name="menu" />
  <Icon name="close" />
  <Icon name="plus" />
  <Icon name="minus" />
  <Icon name="book" />
  </div>
</Article>

<Article theme="lime" class="py-0">
  > Remember, anything new worth taking to market only has to do one job: build new momentum to move your business forward.

  <Figure src="/images/grades-of-innovation.svg">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam ac sapien ac magna tristique auctor. Duis a semper tortor, sit amet laoreet mi. Morbi a lectus id metus faucibus porttitor non id dolor. Duis volutpat, elit quis condimentum gravida, nunc nulla lacinia erat, quis ullamcorper tellus justo quis nulla.</Figure>

  ---
</Article>

<Article theme="yellow" class="py-0">
  > Remember, anything new worth taking to market only has to do one job: build new momentum to move your business forward.

  <Figure src="/images/grades-of-innovation.svg">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam ac sapien ac magna tristique auctor. Duis a semper tortor, sit amet laoreet mi. Morbi a lectus id metus faucibus porttitor non id dolor. Duis volutpat, elit quis condimentum gravida, nunc nulla lacinia erat, quis ullamcorper tellus justo quis nulla.</Figure>

  ---
</Article>

<Article theme='lime'>

`H1`

:::div{.md:hidden}
# Landing pages titles 64/62
:::

:::div{.hidden.md:block}
# Landing pages titles 90/80
:::

`H2`

:::div{.md:hidden}
## Section headings and body titles 32/34
```md
## Section headings and body titles 32/34
```
:::

:::div{.hidden.md:block}
## Section headings and body titles 48/50
```md
## Section headings and body titles 48/50
```
:::

`H3`

:::div{.md:hidden}
### Section headings and body titles 30/30
```md
### Section headings and body titles 30/30
```
:::

:::div{.hidden.md:block}
### Section headings and body titles 48/50
```md
### Section headings and body titles 48/50
```
:::

`H4`

:::div{.md:hidden}
#### Section headings and body titles 21/28
```md
#### Section headings and body titles 21/28
```
:::

:::div{.hidden.md:block}
#### Section headings and body titles 35/44
```md
#### Section headings and body titles 35/44
```
:::

---

`intro`

:::div{.md:hidden}
<p class="lead">Intro mobile 32/36</p>
```html
<p class="lead">Intro mobile 32/36</p>
```
:::

:::div{.hidden.md:block}
<p class="lead">Desktop mobile 48/54</p>
```html
<p class="lead">Desktop mobile 48/54</p>
```
:::

:::div{.not-prose}
<p class="label">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam ac sapien ac magna tristique auctor. Duis a semper tortor, sit amet laoreet mi. Morbi a lectus id metus faucibus porttitor non id dolor. Duis volutpat, elit quis condimentum gravida, nunc nulla lacinia erat, quis ullamcorper tellus justo quis nulla.</p>
:::


<details>
  <summary>Foo</summary>> sdsdsd
</details>

| Name | Mobile | Desktop |
| ---- | ------ | ------- |
| h0   | 72/68  | 110/110 |
| h1   | 64/62  | 90/80   |
| h2   | 35/40  | 48/50   |
| h3   | 30/30  | 64/62   |
| h4   | 21/28  | 35/40   |
| h5   | 18/24  | 24/32   |
| h6   | 18/20  | -       |

---

## Typography

# Heading 1

## Heading 2

### Heading 3

#### Heading 4

##### Heading 5

###### Heading 6

<div class="not-prose">
  <p class="h0">Heading 0</p>
  <p class="h1">Heading 1</p>
  <p class="h2">Heading 2</p>
  <p class="h3">Heading 3</p>
  <p class="h4">Heading 4</p>
  <p class="h5">Heading 5</p>
  <p class="h6">Heading 6</p>
</div>

---

## Blockquote

> Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam ac sapien ac magna tristique auctor. Duis a semper tortor, sit amet laoreet mi. Morbi a lectus id metus faucibus porttitor non id dolor. Duis volutpat, elit quis condimentum gravida, nunc nulla lacinia erat, quis ullamcorper tellus justo quis nulla.

---

## Lists

### Default

- Forward-thinking senior leaders who genuinely want to build a business fit for the future
- Leaders of critical initiatives who want to do everything possible to make them a success
- Open-minded business leaders who want to be challenged to think differently, explore new perspectives and inject new ideas
- Authentic, genuine humans who want to work with authentic, genuine humans – and enjoy the process
- You’re in an established company who wants to become or remain a market leader and is committed to innovation-driven growth
- Or, you’re in a young company who needs to grow from having a great product to become world-class digital business
- You probably have plenty of expertise and in-house skills but you want expert support to complement or supplement what you have

```html
<ul>
  <li>Identifying new opportunities...</li>
  <li>...</li>
</ul>
```

### List 1

:::div{.list-1.not-prose}
Some of the key techniques we use in our work:

- Futurestate visioning
- Commercial strategy
- Service strategy
- Business architecture
- Business model design
- [Experience mapping](/services/experience-mapping)
- Market influences research
- Competitive positioning
- Proposition development
- Ecosystem strategy
- Service mapping
- Brand and messaging
- Content strategy
- Service + Experience design
- Design systems
- Future customer modelling
- Service prototyping
- Technology architecture
- Platform strategy
- Go-to-market strategy
- Growth strategy
- Customer migration
- Innovation culture
- Leadership mentoring
  :::

### List 2

:::div{.list-2.not-prose}

- Embed the digital business practices used by world-leading companies
- Rapidly accelerate your product maturity and align it with real-world commercial strategies
- Teach you vital techniques and shortcuts you won’t find in text books
- Support key decisions with behind-the-scenes insights and lessons learned from leading organisations
- Ensure you avoid common mistakes, hidden pitfalls and costly missteps
- Support you with objective advice when you need it and connect you with people and partners you can trust
  :::

---

Written by: {new Intl.ListFormat('en').format(authors.map(d => d.name))}.

Published on: {new Intl.DateTimeFormat('en', {dateStyle: 'long'}).format(published)}.

:::main{#readme}

Lorem:br
ipsum.

::hr{.red}

A :i[lovely] language know as :abbr[HTML]{title="HyperText Markup Language"}.

:::

## Syntax highlighting

We also support syntax highlighting in MDX out-of-the-box! This example uses our default [Shiki theme](https://github.com/shikijs/shiki). See the [MDX integration docs](https://docs.astro.build/en/guides/integrations-guide/mdx/#syntax-highlighting) for configuration options.

```astro
---
const weSupportAstro = true;
---

<h1>Hey, what theme is that? Looks nice!</h1>
```

## Any questions?

## GFM

### Autolink literals

www.example.com, https://example.com, and contact@example.com.

### Footnote

A note[^1]

[^1]: Big note.

### Strikethrough

~one~ or ~~two~~ tildes.

### Table

| a   | b   |   c |  d  |
| --- | :-- | --: | :-: |

## Tasklist

- [ ] to do
- [x] done

The point can be illustrated by comparing a sample of AsciiDoc with an equivalent sample of Markdown. Here is a sample of AsciiDoc from the AsciiDoc manual:

1. List item one.

- List item one continued with a second paragraph followed by an
  Indented block.
- .................
  $ ls _.sh
  $ mv _.sh ~/tmp
  .................
- List item continued with a third paragraph.

2. List item two continued with an open block.

---

This paragraph is part of the preceding list item.

a. This list is nested and does not require explicit item
continuation.

- This paragraph is part of the preceding list item.

b. List item b.

## This paragraph belongs to item two of the outer list.

## Cat videos

::youtube[Video of a cat in a box]{#kthguiZRzuA}

:::div{.bg-yellow.p-4.lg:p-20}
if you chose xxx, you should also use yyy somewhere…
:::

:::div{.bg-yellow}

## Test

if you chose xxx, you should also use yyy somewhere…

- one
- two
  :::

</Article>
